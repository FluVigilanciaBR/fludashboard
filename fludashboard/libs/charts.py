...
def ethio_ts(df=pd.DataFrame, scale_id=int, year=int):
    ...
    trace.append(
        go.Scatter(x=df['epiweek'], y=df[cols[0]], name=cols[0], mode='lines',)
    )
    ...
    for i in range(1, (nrows + 1)):
        ...
        fig['layout'][yaxis].update(
            range=[0, ymax[min(1, i - 1)]], rangemode='nonnegative'
        )

    fig['layout']['xaxis%s' % nrows].update(
        title='Semana epidemiológica', zeroline=True, showline=True
    )
