...
class FluDB:
        ...
        if show_historical_weeks:
            with self.conn.connect() as conn:
                epiyearmax = conn.execute(
                    'SELECT MAX(epiyear) '
                    'FROM current_estimated_values%(table_suffix)s' % sql_param
                ).fetchone()[0]
            if year < epiyearmax:
    ...
    def get_data_age_sex(
        ...
        df_age_dist = self.read_data(
            'clean_data_epiweek_weekly_incidence_w_situation%s'
            % table_suffix[filter_type],
            dataset_id=dataset_id,
            scale_id=scale_id,
            year=season,
            territory_id=territory_id,
    ...
    def get_opportunities(
        ...
        sql = (
            '''
        SELECT
        delay.symptoms2hospitalization as
        "Primeiros sintomas à hospitalização",
        delay.hospitalization2evolution as
