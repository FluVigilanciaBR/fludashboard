...
scale_id = {'incidência': 1, 'casos': 2}

# ### 1.3 Situation

situation_id = {'unknown': 1, 'estimated': 2, 'stable': 3, 'incomplete': 4}

# ### 1.4 Territory Type

territory_type_id = {'Estado': 1, 'Regional': 2, 'Região': 3, 'País': 4}
...
def migrate_from_csv_to_psql():
    ...
    dfs[dataset].loc[regions_indeces, 'territory_id'] = (
        dfs[dataset].loc[regions_indeces, 'territory_id'].map(region_id)
    )
    ...
    # remove unnecessary fields
    dfs[dataset].drop(
        ['state_country_name', 'territory_type'], axis=1, inplace=True
    )
    ...
    migration_rules = {
        ...
        'Média geométrica do pico de infecção das '
        'temporadas regulares': 'geom_average_peak',
        ...
        'IC inferior (2,5%)': 'epi_start_ci_lower',
        'SE típica do início do surto - '
        'IC superior (97,5%)': 'epi_start_ci_upper',
        'duração típica do surto': 'epi_duration',
        'duração típica do surto - '
        'IC inferior (2,5%)': 'epi_duration_ci_lower',
        'duração típica do surto - '
        'IC superior (97,5%)': 'epi_duration_ci_upper',
        'temporadas utilizadas para os '
        'corredores endêmicos': 'regular_seasons',
        'ano': 'year',
        'Unidade da Federação': 'state_country_name',  # Not needed ...
        'Tipo': 'territory_type',  # Not needed in the table
        'dado': 'dataset_id',
    ...
    df_dataset = pd.DataFrame(
        {'id': list(dataset_id.values()), 'name': list(dataset_id.keys())}
    ).set_index('id')
    ...
    df_situation = pd.DataFrame(
        {'id': list(situation_id.values()), 'name': list(situation_id.keys())}
    ).set_index('id')
    ...
    df_scale = pd.DataFrame(
        {'id': list(scale_id.values()), 'name': list(scale_id.keys())}
    ).set_index('id')
    ...
    for k, df in dfs.items():
        ...
        df.to_sql(k_new, engine, if_exists='replace', chunksize=2048)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Update datafiles and DB.\n",
        formatter_class=RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        '-F', '--Force', help='Force download', action='store_true'
    )
    parser.add_argument(
        '-d', '--database', help='Update database', action='store_true'
    )
